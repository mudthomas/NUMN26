...
class BDF_general_Newton(FPI.BDF_general):
    ...
    def __init__(self, problem, order):
        """Initiates an instance of BDF_general_Newton.
    ...
    def integrate(self, t0, y0, tf, opts):
        ...
        for i in range(self.order, self.maxsteps):
            if t >= tf:
                break
            self.statistics["nsteps"] += 1
            t, y = self.BDFstep_Newton(t, y_list[-self.order:][::-1], h)
            t_list.append(t)
            y_list.append(y)
            h = min(h, np.abs(tf - t))
    ...
    def BDFstep_Newton(self, t_n, Y, h, coeffs=None):
        ...
        y_np1_ip1, info, success = opt.fsolve(lambda x: static_terms + coeffs[-1] * h * self.problem.rhs(t_np1, x) - x,
                                              y_np1_i, xtol=self.tol, maxfev=self.maxit, full_output=True)[:3]

        if success:
            self.statistics["nfcns"] += info["nfev"]
            return t_np1, y_np1_ip1
        else:
            raise Explicit_ODE_Exception(f'Corrector could not converge within {self.maxcount} steps')
